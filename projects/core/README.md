# BOLT-CORE – Angular 19 CORE (@cadai/psx-ng-core)
...
- 🦾 **CI/CD** with GitLab CI and GitLab Package Registry
...
- ✅ **CI/CD-ready** with GitLab CI support

## 6. 📥 Consuming the Library in Apps

The package is published to this project's **GitLab Package Registry**.

In the host Angular app, configure npm for the `@cadai` scope:
...
For local development, set `NPM_TOKEN` to a GitLab Deploy Token or Personal Access Token with `read_package_registry`.

For another GitLab CI project, prefer a masked CI/CD variable or a Deploy Token with `read_package_registry`. If cross-project `CI_JOB_TOKEN` access is enabled in GitLab, this can also work:
...
Host apps must provide the peer dependencies declared by the SDK, especially Angular, NgRx, Translate, Keycloak, RxJS, Chart.js and Luxon.
...
| -------- | ------------------------------------------------------------------------------------ | -----: | -------- | -------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------ | --------------------------------------------------------- | ----- |
| Core     | Barrels and config                                                                   |     ✅ | P1       | XL       | App uses standalone components, strict TS/ESLint. Also imports are done via barels `@cadai/pxs-ng-core/*`                      | `See GitLab repository and Package Registry`     | —                                                         | FE    |
...
| Core     | CI/CD                                                                                |     ✅ | P1       | M        | Automatic builds, CI version bumping, package publishing and version tags                                                      | `.gitlab-ci.yml`                                 | Validate publish in GitLab Package Registry               | FE    |
...
| Core     | Versionning                                                                          |     ✅ | P1       | S        | CI bumps `projects/core/package.json` before publish, then tags `PXS-NG-CORE@<version>`                                        | `.gitlab-ci.yml`                                 | Use `RELEASE_BUMP` for non-patch releases                 | FE    |
