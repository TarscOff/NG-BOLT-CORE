...
```ts
// types
type InspectorActionType = 'chat-basic' | 'chat-on-file' | 'compare' | 'summarize' | 'extract';
...
>
  ...
  <button
    *ngIf="visualType()!=='input' && visualType()!=='result'"
    mat-mini-fab
    class="neutral"
...
- **Edges don’t start at the bullet** → don’t offset the _rails_; center bullets with CSS transform on `.df-connector` or use connector `[offsetX]/[offsetY]` if available.
